...
def export(
...
) -> Dict[str, Union[csr_matrix, Series, BaseEstimator]]:
    df, df_train, df_val = data
    target = kwargs.get('target', 'duration')

    # Vectorizing the features for the entire dataset and training/validation sets
    ...
    # Returning the values in a dictionary format
    return {
        'X': X,
        'X_train': X_train,
        'X_val': X_val,
